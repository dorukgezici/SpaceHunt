...
export default class RockCreator {
	...
	constructor(engine: ex.Engine, scene: ex.Scene, bounds: ex.BoundingBox, level: Level3, rocks: Rock[]) {
		this.engine = engine;
		this.scene = scene;
		this.bounds = bounds;
		this.level = level;
	...
	createNewRockRT(scene: ex.Scene, bounds: ex.BoundingBox, level: Level3, rocks: Rock[], timer: number[]) {
		...
		if (this.randomIntFromInterval(1, 2) > 1) {
			// big rock
			rock = new Rock(level.frontPlayer.x + 800, 500, 50, -250, 1000, -550, Rocktypes.big, this.level.pubPlayers); // big rock bouncing towards the player		
		} else {
			// small rock
			rock = new Rock(level.frontPlayer.x + 800, 500, 20, -250, 1000, -250, Rocktypes.small, this.level.pubPlayers); // small rock "rolling" on the ground
		}
